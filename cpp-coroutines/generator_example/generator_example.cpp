...
{
    for ( T i = {};; ++i ) {
        std::cout << __FUNCTION__ << " yield value: " << i << "\n";
        co_yield i;
    }
}
...
generator<T> take_until( generator<T> & g, T limit )
{
    for ( auto && v : g )
        if ( v < limit ) {
            std::cout << __FUNCTION__ << " yield value: " << v << "\n";
            co_yield v;
        } else
            break;
}
...
generator<T> multiply( generator<T> & g, T factor )
{
    for ( auto && v : g ) {
        ...
        std::cout << __FUNCTION__ << " yield value: " << v * factor << "\n";
...
{
    for ( auto && v : g ) {
        std::cout << __FUNCTION__ << " yield value: " << v + adder << "\n";
        co_yield v + adder;
    }
}
